...
N_PAGES = 0 # number of pages of search results per keyword, n_pages * 50 will be the number of video results per term
MAX_RESULTS = 50 # number of results per search from 0-50

video_keywords = ["college"] #testing smaller search term set
            ...
            for i in range(N_PAGES):
                ...
                for item in items:
                        ...
                        if video_id not in cached_video_ids:
                            ...
                            raw_description = yt.get_video_description_raw(video_response)
                                ...
                                for dKey in description_keywords:
                                    if dKey in desc:
                                        ...
                                        if not false_pos:
                                            ...
                                            emails = yt.parse_description_for_email(raw_description)
                                            keyword_hits[video_id] = {"vKey": vKey, "dKey": dKey, "desc": desc, "emails": emails,"title": vidTitle, "channelTitle": channelTitle}
                                            hit_count += 1
